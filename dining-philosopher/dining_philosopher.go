...
func diningProblem(philopher string, leftFork, rightFork *sync.Mutex) {
	...
	for i := hunger; i > 0; i-- {
		...
		time.Sleep(sleepTime)
		leftFork.Lock()
		fmt.Printf("\t%s picked up the fork to his left.", philopher)
		rightFork.Lock()
		fmt.Printf("\t%s picked up the fork to his right.", philopher)
		fmt.Println(philopher, "has both forks, and is eating.")
		time.Sleep(eatTime)
		rightFork.Unlock()
		fmt.Printf("\t%s put down the fork on his right.", philopher)
		leftFork.Unlock()
		fmt.Printf("\t%s put down the fork on his left.", philopher)
		time.Sleep(sleepTime)
	}
